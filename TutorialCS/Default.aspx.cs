...
public partial class _Default : System.Web.UI.Page 
{
    ...
    {
        SqlDataAdapter da = new SqlDataAdapter("SELECT DISTINCT A.ID_EVENTO AS ReservationId,B.RUT_NUM,LTRIM(RTRIM(B.NOMBRE))+' '+LTRIM(RTRIM(B.A_PAT))+' '+ LTRIM(RTRIM(b.A_MAT)) AS ReservationName,C.ID_CAMA AS RoomId, A.ID_ESTADOCAMA AS ReservationStatus,A.FECHAINI as ReservationStart,A.FECHA_FIN AS ReservationEnd FROM HOS_GESTIONCAMA A, PACIENTE B,CAMA C  WHERE A.RUT_NUM = B.RUT_NUM AND A.ID_CAMA = C.ID_CAMA AND A.ID_ESTADOCAMA = A.ID_ESTADOCAMA AND NOT A.ID_ESTADOCAMA=8 AND NOT (([FECHA_FIN] <= @start)OR ([FECHA] >= @end)) AND A.EVENTOPAC=1 AND A.ID_ESTADOCAMA NOT BETWEEN 9 AND 11", ConfigurationManager.ConnectionStrings["cnsicbo"].ConnectionString);
